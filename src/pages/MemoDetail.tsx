...
const mockMemoDetail = {
  ...
  description:
    "Scheduled system maintenance for core banking applications will take place over the weekend. All departments are advised to save their work and log out by Friday 6 PM.",
};
...
const mockApprovalSteps: ApprovalStep[] = [
  {
    ...
    status: "approved",
    timestamp: "Jan 26, 2025 at 9:00 AM",
    signatureVerified: true,
    comment: "Maintenance window confirmed with vendor. Proceeding with notification.",
  },
  {
    id: "step-2",
    order: 2,
    ...
    status: "approved",
    timestamp: "Jan 26, 2025 at 11:45 AM",
    signatureVerified: true,
    comment: "Approved. Please ensure all critical backups are completed before maintenance.",
  },
  {
    id: "step-3",
    order: 3,
    ...
    status: "pending",
    timestamp: "Awaiting action since Jan 26, 2025",
  },
  {
    id: "step-4",
    order: 4,
    ...
    <AppLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => navigate("/memos")}>
            <ArrowLeft className="h-5 w-5" />
          </Button>
          <div className="flex-1">
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="text-2xl font-bold text-foreground">{memo.id}</h1>
              {getOverallStatusBadge(memo.status)}
              {getPriorityBadge(memo.priority)}
            </div>
            <p className="text-muted-foreground mt-1">{memo.subject}</p>
          </div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <Card className="border-border lg:col-span-1">
            ...
            <CardContent className="space-y-4 text-sm">
              {[
                { icon: Mail, label: "From", value: memo.from },
                { icon: Users, label: "To", value: memo.to },
                { icon: Building2, label: "Department", value: memo.department },
                { icon: User, label: "Created By", value: memo.createdBy },
                { icon: CalendarDays, label: "Date Created", value: memo.createdAt },
              ].map((item) => (
                ...
                <>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-foreground">{memo.description}</p>
                    </div>
                  </div>
                </>
              )}
